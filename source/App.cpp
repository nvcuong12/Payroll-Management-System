...
void App::handleDeleteEmployee(int contentX, int contentY, int contentWidth, int contentFrameY2) {
    ...
    if (_empManager.deleteEmployee(empId)) { 
        saveData(); 
        gotoXY(contentX + tab, contentFrameY2-2); std::cout << "Da xoa thanh cong NV " << empId << ". Du lieu da luu.";
    } else {
        gotoXY(contentX + tab, contentFrameY2-2); std::cout << "Khong tim thay NV " << empId << ". Xoa that bai.";
    }
    cDelay(2000);
}
...
bool App::handleLogin() {
    ...
    while (attempts < 3) {
        ...
        password_str = ""; 
        while (true) {
            ch_input_raw = getch(); // Đọc ký tự đầu tiên

            if (ch_input_raw == 0 || ch_input_raw == 0xE0) { // Nếu là ký tự đặc biệt (mũi tên, F-keys,...)
                getch(); // Đọc và bỏ qua byte thứ hai của ký tự đặc biệt
